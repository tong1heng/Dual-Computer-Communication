...
//    addrSrvMsg.sin_addr.S_un.S_addr = inet_addr("127.0.0.1");

    //创建客户端套接字
    sockClient = socket(AF_INET, SOCK_STREAM, 0);   //流步套接字
    if((unsigned long long)SOCKET_ERROR == sockClient) {
    ...
    //向服务器发出连接请求
    if((unsigned long long)::connect(sockClient, (struct sockaddr*)&addrSrvMsg, sizeof(addrSrvMsg)) == INVALID_SOCKET) {
    ...
    //开一个不断循环等待接收消息的线程
    pthread_t tidsMsg[2];
    int retMsg = pthread_create(&tidsMsg[0], NULL, ctrlRecvMsg, (void*)&sockClient); //参数：创建的线程id，线程参数，线程运行函数的起始地址，运行函数的参数
...
void client::on_sendMessageButton_clicked() {
    ...
    int e = send(sockClient, buff, ba.size(), 0);
    if (e == SOCKET_ERROR) {
