...
#include "ui_serverAudio.h"

serverAudio::serverAudio(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::serverAudio)
{
    ui->setupUi(this);
    ...
    udpSocketSend->bind(QHostAddress::Any,10004);   //绑定10004端口
    //设置采样参数
    ...
    udpSocketReceive->bind(QHostAddress::Any,10005);
...
void serverAudio::onReadySend() {
    ...
    udpSocketSend->writeDatagram((const char*)&vp,sizeof(vp),destaddr,10004);   //将此结构体发送给目标主机，端口10004
}

void serverAudio::onReadyReceive() {
...
void serverAudio::receiveAddress(QString addr) {
    address=addr;
}

void serverAudio::on_exitButton_clicked() {
    emit endChat();
    this->close();
}
